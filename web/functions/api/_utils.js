...
async function normalizeStoredUsers(users, defaultAdmin, defaultAdminPassword = "") {
  ...
  for (const user of source) {
    const fallbackPassword =
      normalizeValue(user?.username).toLowerCase() === normalizeValue(defaultAdmin.username).toLowerCase()
        ? normalizeValue(defaultAdminPassword)
        : "";
    const next = { ...user };
    ...
    const storedPasswordHint = normalizeValue(next.password_hint);
...
async function ensureUsers(env) {
  ...
  users = await normalizeStoredUsers(users, defaultAdmin, defaultPassword);
