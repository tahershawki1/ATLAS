const app = {
  ...
  mergeCompanies(baseCompanies = [], customCompanies = []) {
    ...
    customCompanies.forEach((customCompany) => {
      ...
      if (target) {
        target.plots = [
          ...(target.plots || []),
          ...this.deepClone(customCompany.plots || []),
        ];
  ...
  init() {
    ...
    this.state.customSiteDb = this.loadCustomSiteDb();
  ...
  addNewSiteCompany() {
    const companyName = window.prompt("اكتب اسم الشركة الجديدة");
  ...
  addCompanyReference(fieldName) {
    ...
    const referenceValue =
      currentValue || this.normalizeValue(window.prompt(`أدخل ${label} الجديدة`));
